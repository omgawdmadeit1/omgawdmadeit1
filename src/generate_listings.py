...
def build_quote_range(item: Item, pricing_config: dict, logger: logging.Logger) -> str:
    low, high = compute_quote_values(item, pricing_config, logger)
...
def build_follow_up_messages(
    ...
    next_action = "ready_for_approval" if confidence_score >= 0.9 else "needs_info"
    summary = f"Summary: {item.title_hint} ({item.condition})"
    quote_low, quote_high = compute_quote_values(item, pricing_config, logger)
    estimate_range = f"${quote_low:,.2f}-${quote_high:,.2f}"
    estimate = f"Estimate range: {estimate_range}"
    time_line = ""
    if next_action == "ready_for_approval":
    ...
    if time_line:
        friendly_lines.append(time_line)
        direct_lines.append(time_line)
    friendly_lines.append("Happy to help whenever you're ready.")
    if questions_block:
        friendly_lines.append(questions_block.strip())
        direct_lines.append(questions_block.strip())
    friendly = "\n".join(friendly_lines)
    direct = "\n".join(direct_lines)
    ...
    return {
        ...
        "quote_low": f"{quote_low:,.2f}",
        "quote_high": f"{quote_high:,.2f}",
...
def write_pending_quotes(
    ...
    for listing in listings:
        ...
        recommended_style = (
            "direct"
            if listing.follow_up_messages.get("next_action") == "ready_for_approval"
            and listing.confidence_score >= 0.8
            else "friendly"
        )
            ...
            writer.writerow(
                {
                    ...
                    "quote_low": payload.get("quote_low", ""),
                    "quote_high": payload.get("quote_high", ""),
